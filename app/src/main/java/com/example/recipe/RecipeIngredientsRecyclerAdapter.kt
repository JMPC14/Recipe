...
class RecipeIngredientsRecyclerAdapter(private val context: Context, private val recipe: RecipeInfo) : RecyclerView.Adapter<RecipeIngredientsRecyclerAdapter.ViewHolder>() {
            ...
            itemView.ingredient_Name.setOnLongClickListener {
                ...
                pop.setOnMenuItemClickListener { item ->
                    when(item.itemId)
                    {
                        R.id.Edit -> {
                            ...
                            okButton.setOnClickListener {
                                ...
                                if (newName.isEmpty()) {
                                    ...
                                    RecipeManager.recipes[find].ingredients[ingredientPosition] = newName
                        ...
                        R.id.Remove -> {
                            RecipeManager.recipes[find].ingredients.removeAt(ingredientPosition)
                            RecipeManager.recipes[find].quantity.removeAt(ingredientPosition)
                            notifyDataSetChanged()
                        }
                            ...
                            okButton.setOnClickListener {
                                ...
                                if (newQuantity.isEmpty()) {
                                    ...
                                    if (add != "" && (newQuantity.toFloatOrNull() == null || newQuantity.toFloat() < 0)) {
                                        ...
                                        RecipeManager.recipes[find].quantity[ingredientPosition] =
                                            "${newQuantity}${add}"
                                        notifyDataSetChanged()
                                        hideKeyboardFrom(context, dialogView)
                                        newDialog.cancel()
                        ...
                        R.id.Buy -> {
                            val ingredient = RecipeManager.recipes[find].ingredients[ingredientPosition]
                            val quantity = RecipeManager.recipes[find].quantity[ingredientPosition]
    ...
    override fun getItemCount() = recipe.quantity.size

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val ingredient = recipe.ingredients[position]
        val quantity = recipe.quantity[position]
